...
func (s *Server) handleKnowledgeItems(w http.ResponseWriter, r *http.Request) {
	...
	options := domain.KnowledgeListOptions{Status: status, Kind: r.URL.Query().Get("kind"), Visibility: domain.KnowledgeVisibility(r.URL.Query().Get("visibility")), AfterID: r.URL.Query().Get("cursor"), Limit: 100}
