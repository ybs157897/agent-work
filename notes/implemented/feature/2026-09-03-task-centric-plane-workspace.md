...
任务工作区采用 [makeplane/plane](https://github.com/makeplane/plane) 的信息层级：列表或看板上的一级对象只有 Task，详情页先展示任务标题、描述、子任务与属性，再在任务内部展示执行 Agent。

列表不再并列展示复审队列、决策、产物、Coordinator 状态或执行锁；搜索也只匹配当前 Task 集合。详情页不再把 Coordinator、Goal/Todo/Plan、派发批次、技术日志、交付简报和运行产物抬成与任务平级的面板。控制面和治理数据仍由原 store/API 保留，本次只收敛用户可见投影。
...
- Task 列表只显示 `record_kind=task` 的记录及其任务属性，不显示 Chat、决策、产物或运行时对象；`cancelled` 仍是可追踪的任务列，不会被过滤消失。
