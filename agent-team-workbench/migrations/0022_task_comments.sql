-- 0022_task_comments.sql — Task 评论流与验收读模型字段（SQLite）。
-- JSON 字符串以 json_type(x,'$.k')='text' 识别；数组遍历使用 WITH RECURSIVE
-- 生成下标和 '$.k[i]' 路径；pending_instruction 通过
-- json_remove(data,'$.pending_instruction') 删除而非置 null。
-- 回填口径：cursor 回填、acceptance_criteria 仅从 state.data / plan step payload
-- 的字符串数组逐字回填（否则置 NULL，不从
