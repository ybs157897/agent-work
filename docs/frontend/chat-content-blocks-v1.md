...
用户要的产出本身就是一份文档时（需求规格、PRD、方案、评审报告、notes 决策记录）：文档按仓库
约定落盘留痕，并用带 `path` 的 `file` 块指向它；用户在对话里点开文件行，在抽屉里读渲染后的
Markdown。

- 正文给结论、要点与待裁决项，不整篇抄进正文——正文精简纪律对交付型文档照旧适用。
- `file` 块必须给出可解析的仓库相对路径（`path`，并让 `name` 与之一致）：只给文件名、只给
  `file://` URL，或干脆不给路径，都等于没交付。
- 文档无法落盘、或所在位置不在可读集合内（例如远程执行宿主没有仓库目录）时，才把完整正文写进
  消息正文，并在正文里说明为什么没有可打开的文件。
