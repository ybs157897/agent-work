...
export function AppShellSkeleton({ chat = false }: { chat?: boolean }) {
  ...
  const theme = useWorkbenchThemeStore.getState().theme;
  return (
    <div
      className={`workbench-theme flex h-dvh w-full flex-row overflow-hidden ${chat ? 'bg-sidebar' : 'bg-surface-base'}`}
      data-theme={theme}
      role="status"
      aria-label="正在连接控制平面"
    >
      <div className="workbench-sidebar h-full shrink-0 space-y-3 border-r border-sidebar-border bg-sidebar p-4">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <div className={chat ? 'workbench-chat-surface min-h-0 flex-1' : 'flex-1 space-y-comfortable p-comfortable'}>
          {chat ? null : (
            <>
