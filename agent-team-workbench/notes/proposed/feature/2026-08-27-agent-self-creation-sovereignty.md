...
讨论时间线：2026-08-27 与用户对谈沉淀（辅助会话起草）；同日主线会话补充五节
（Cordis 分界、表达力阶梯、空间数据面、残余风险、分期演进、备选与否决）。
三个话题拼的是同一个东西的三个面——
...
对照 cordiverse/cordis（DSH 底层 TS 插件框架）：它解决**单进程内插件组装**
（ctx 容器 / inject 依赖图 / reload 重建实例 / 可逆副作用），我们解决**跨进程
多 harness 持久编排**——不同类问题，三条分界：

1. **热载发生在哪**：Cordis 热载进 harness（执行面），不进 orchestrator；我们
   同构——agent 写的代码只活在子进程，"agent 写 TS 插件热载进 harness"的体验
   经 dsh adapter 今日已有。编排器永不热载，不是 Go 做不到，是任何语言都不该开
   （引擎语义被模型改写 = 护栏失效）。
2. **组合单元**：Cordis 插件=同进程 Service 对象（要实例化/挂载/解析依赖）；
   我们 agent=数据（四轴配置），创建即用、零加载仪式——在"agent 是配置"前提
   下更少仪式感。
3. **失败哲学**：Cordis 可逆副作用（reload 可撤销的世界）；我们不可逆审计
   （墓碑 / 终态唯一 / 任何 Outcome 必落终态）——持久编排选后者。
...
- **近期（画框一等化）**：`created_by` 谱系 + recruit/retire 词汇（contracts
  版本化）+ bundle schema + onboarding 目录约定（先约定后强制）；
- **中期（框成型）**：预算信封与 lineage 聚合、空间目录配额与 artifact 唯一
  出口、自改入职包闭环（审计事件 + 首改审批）、子树 lead 执行路径验证；
- **远期（框生态）**：意识信号自动化（dispatch 相似度聚类）、按风险分级免审、
  atw-mcp 退役与认领模式归并。
