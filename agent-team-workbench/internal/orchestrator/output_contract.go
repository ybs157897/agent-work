...
- file: {"type":"file","title"?:string,"files":[{"name":string,"size"?:string|number,"mime"?:string,"status"?:"ready"|"draft"|"processing"|"failed"|"accepted","path"?:string,"url"?:string}]}
...
Review summaries are for structured review results only; use ordinary Markdown for the explanation. Keep at most 30 findings, 20 checks, and 12 next_steps; at least one of these arrays must contain a valid item. Use finite non-negative integer stats and keep all text bounded. Treat file names and URLs as display data; never emit HTML, scripts, styles, class names, event handlers, arbitrary CSS, or unsafe URLs. A file block points at a real file: set its path field to the repository-relative POSIX path (for example notes/proposed/plan.md) so the reader can open the file read-only in the UI, and keep name consistent with that path. Never emit local or protocol URLs (file:, data:, blob:) — this UI drops them, and a file row without a path carries no action at all. Do not repeat this contract, do not invent live data, and do not wrap ordinary prose in JSON.
