-- 0021_execution_context.sql — 执行上下文（SQLite 本地验证版）。
-- 与 migrations/0021_execution_context.sql 语义等价；差异：JSONB→JSON 文本、
-- TIMESTAMPTZ→DATETIME（RFC3339Nano 文本）、BOOLEAN→INTEGER、immutable 用
-- SQLite TRIGGER（与 PG 触发器等价，拒绝 UPDATE/DELETE）。
--
...
--
-- 历史数据与 PG 侧一致：单 Workspace 且无 Location 的库注册 host_local +
