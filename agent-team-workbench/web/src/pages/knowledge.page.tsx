  ...
  return (
    <main className={cx('page-shell knowledge-reading-page', urlState.itemId ? 'knowledge-reader-open' : 'knowledge-reader-closed')} data-testid="knowledge-reading-layout" aria-label="知识库阅读工作区">
      <header className="page-header">
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-tight"><StatusPill title="当前工作区"><BookOpen className="h-3.5 w-3.5 text-brand-primary" aria-hidden="true" /><span className="max-w-44 truncate">{workspace?.name ?? workspaceId}</span></StatusPill><Button type="button" onClick={() => void refreshAll()} disabled={refreshing} aria-label="刷新知识库"><RefreshCw className={cx('h-4 w-4', refreshing && 'animate-spin')} aria-hidden="true" />刷新</Button></div>
      </header>
