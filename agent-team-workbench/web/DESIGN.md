...
空泳道默认收起，以可点击的状态索引保留可发现性；少量非空泳道适配可用宽度，窄窗口保留看板区域内横向滚动，列表作为紧凑查看方式。Task 局部使用 Plane 浅色语义 token，辅助文字不小于 12px，正常文字对比度不低于 4.5:1，完成任务不划删除线。任务详情 Drawer 和打回 Modal 均使用 `skin="task"`，不能因 portal 脱离页面节点而回落到另一套皮肤。
...
Agent 历史请求失败须在该 Agent 的最终输出区提供明确错误和局部重试，不冒充加载中、空结果或最终正文，也不影响其他 Agent 阅读。新建任务只发布总任务，不暴露父子层级选择；必填说明初始为中性，校验在失焦或提交后显示。任务对话在离开页面和刷新后保留内容，发布后保留该轮对话与任务链接；打回表单关闭后保留当前草稿、提交成功才清除，草稿按任务隔离，普通任务刷新不清空。任务切换后到达的旧请求不关闭新任务详情。

侧边栏常驻“任务对话”，打开独立 `/task-chat` 全高页面；任务看板的新建按钮也跳转到该页。页面不使用创建 Drawer 或遮罩，采用 `tx-scope chat-languagegui-skin`、用户消息卡和既有 composer 阅读轨，assistant 正文只使用 `AgentOutput`。用户先自由描述想法，模型追问关键缺口，形成可编辑的标题、任务描述与验收标准。发送消息只分析需求；只有“确认发布”才能调用原总任务创建流程。继续对话后须重新确认最新草案，不能误发之前版本。对话、草案和失败信息按工作区隔离；切换页面和刷新均保留。发布成功后留在页面，正文展示“已发布”及查看任务链接，保留本轮只读对话并移除发布按钮；显式开始新任务对话后才能再次澄清和发布。分析模型默认沿用任务设置，也可选择已配置的文本模型；显式切换后重试应使用新选择。超限输入在本地拒绝并保留原文；“重新开始”须经 Modal 确认，发布结果未确定时不能清除。用户向上阅读历史时不自动滚到底部。未支持的附件、工具和 Agent 调度能力不出现在输入区。

任务澄清的正文问答使用结构化 `questions`，不解析自然语言编号生成按钮。每题独立垂直排列，依据题目语义使用单选或多选，并始终允许自由补充；选择本身不发送请求，“提交回答”才继续分析。已提交或已被新一轮取代的问题保持只读，完整问题与答案留在对话历史。`AgentOutput` 继续只负责正文，问答控件由外层传入受控状态与回调。欢迎说明仅在空态显示，分析模型与重新开始使用紧凑工具行，避免重复说明和大卡片挤占正文空间。
...
知识库管理员是 `kind=knowledge_librarian` 的系统内置 Agent，固定身份和职责，不由用户从普通成员中选择。它出现在通用聊天选择器和 `@` 候选中，与其他可聊天 Agent 共用 `/chat`、会话、Run 时间线和 `AgentOutput`；不创建专属聊天页，不用结构化控制 JSON 替代自然语言回复。`task_coordinator` 和未知系统身份仍不进入普通聊天。

`/agents?agent=<id>` 为内置管理员提供模型和运行方式设置；名称、角色、提示词和权限由系统维护，不通过普通编辑请求提交。知识页、设置页可以直接打开管理员的通用聊天。后台用于检索/整理的 KnowledgeJob 继续是内部执行结构，不混入用户普通聊天列表或暴露为可继续发送消息的对话。

`/knowledge` 只负责看内容：浏览、搜索、阅读正文，查看来源、关系和版本。正文复用 `AgentOutput`。页面没有资料录入、发起调查、选择管理员、人工整理或发布表单；空态说明团队成员会在对话中将需保留的内容交给管理员。需要询问或补充时，入口前往通用聊天，而不是展开知识操作台。

典型流程为“用户与产品 Agent 讨论并确认需求 → 产品 Agent 调用内置管理员 → 管理员整理并返回真实入库结果 → 用户到知识库阅读”。来源、关系和适用条件由 Agent 组织；未确认或证据不足的内容通过对话反馈，知识状态继续以服务端事实为准，不能把模型口头说已记录当作写入成功。

设置页保留工作目录连接恢复。目录迁移造成绑定代际变化时，仅允许重新连接到当前已广告且项目身份相同的目录；不接受任意路径，不绕过服务端版本和代际校验。对话发送失败保留原文与实体幂等键，配置后返回同一对话不清除待发内容；Kimi 后台和 WaitFor 完成继续由同一 Run 跟踪。未发送队列仍为内存状态，刷新或切换工作区不承诺恢复。
