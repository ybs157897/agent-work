...
交付面口径（同日追加定调）：**正文 md 与带 `path` 的文件卡都算交付**，二者可并存；抽屉是文件卡那条路
的阅读面，不是唯一交付形态。唯一不成立的交付是「只给一个打不开的指针」（裸文件名、`file://`、可读集合
之外的位置）。
...
- **强制交付型文档必须落盘并只给文件卡**：2026-09-13 当天被用户改回「正文 md 也算交付」。正文本来就是
  Markdown 渲染面，读者在对话里直接读完整文档是合法选择；被否决的是"必须写进正文、不许给文件卡"那套强制。
