...
it('未创建 Run 的发送失败有常驻原因、原文保留说明和准确配置入口', () => {
  const html = renderToStaticMarkup(<MemoryRouter><SendErrorNotice message="runtime binding unavailable" agentId="agent_atlas" /></MemoryRouter>);
  expect(html).toContain('role="alert"');
  expect(html).toContain('消息暂未发送，原文已保留');
  expect(html).toContain('href="/agents?agent=agent_atlas"');
