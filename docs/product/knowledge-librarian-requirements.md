...
本契约只规定产品行为和验收面。正文存储、索引实现、Harness 的进程接法，以及普通查询和复杂查询之间的具体路由，仍由主线架构设计确认。

本契约延续既有知识层方向：workspace 级知识目录、KnowledgeRetriever 抽象、Agent 通过 consult_knowledge 消费知识。[end-goal.md:119-121] 知识是共享领域事实，角色提示词定义 Agent 是谁以及如何工作。[knowledge/README.md:1-3]
...
- 允许源权限按 Agent、角色、workspace、project 和知识分类配置。
...
- verified：有已验收 artifact、测试结果、已生效条目或批准文档直接支持；
- observed：run 中观察到且有定位，但尚未完成领域确认；
- inferred：由 Agent 或管理员综合推断；
- unverified：缺少可复核来源。

只有 verified 或经 owner 明确批准的内容才可能进入 effective；observed、inferred 和 unverified 默认留在 candidate/draft。
...
| 有强证据且不改变规范语义的事实更新 | 可由显式策略允许的 owner 批量确认或自动发布 |
...
普通查询和复杂查询的阈值、是否调用管理员 LLM、以及 Harness 的具体路由由主线架构确认；本契约只要求两种路径都遵守同一覆盖、证据和终止规则。
...
以下问题是实现前的设计门，本需求文档不代答：

1. 查询 Harness 的调用协议、超时、并发和预算如何定义；
2. 普通查询和复杂查询的判定信号，以及何时调用管理员 LLM；
3. 混合召回各通道的优先级、可解释性和降级行为；
4. 版本、关系、索引和知识包发布的原子边界；
5. source policy 的配置者、继承规则和跨 workspace/project 的授权模型。
